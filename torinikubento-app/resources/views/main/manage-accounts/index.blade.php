            ...
            <div class="flex flex-col lg:flex-row gap-3">
                ...
                <div class="flex-1 lg:max-w-md">
                    <div class="relative">
                        <input type="text" id="searchInput" placeholder="Cari nama, username, atau email..." 
                               class="w-full bg-neutral-700 text-white placeholder-neutral-400 rounded-lg pl-10 pr-4 py-2.5 border border-neutral-600 focus:border-orange-500 focus:ring-1 focus:ring-orange-500/20 focus:outline-none transition-all duration-200 text-sm">
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                    ...
                    <div class="min-w-0 sm:min-w-[140px]">
                        <select id="roleFilter" class="w-full bg-neutral-700 hover:bg-neutral-600 text-white rounded-lg px-3 py-2.5 border border-neutral-600 focus:border-orange-500 focus:ring-1 focus:ring-orange-500/20 focus:outline-none transition-all duration-200 text-sm">
                    ...
                    <div class="min-w-0 sm:min-w-[120px]">
                        <select id="statusFilter" class="w-full bg-neutral-700 hover:bg-neutral-600 text-white rounded-lg px-3 py-2.5 border border-neutral-600 focus:border-orange-500 focus:ring-1 focus:ring-orange-500/20 focus:outline-none transition-all duration-200 text-sm">
                    ...
                    <button type="button" id="clearFilters" 
                            class="px-4 py-2.5 bg-neutral-600 hover:bg-neutral-500 text-white rounded-lg transition-colors duration-200 text-sm font-medium whitespace-nowrap">
            ...
            <table class="w-full" id="usersTable">
                ...
                <tbody class="divide-y divide-neutral-700/30">
                    @forelse($users as $user)
                        <tr class="hover:bg-neutral-700/20 user-row" 
                            ...
                            <td class="px-4 py-2">
                                <span class="px-2 py-1 rounded text-xs font-medium {{ $user->is_active ? 'bg-green-600/10 text-green-400' : 'bg-red-600/10 text-red-400' }}">
                            ...
                            <td class="px-4 py-2">
                                ...
                                <x-ui.context-menu id="user-menu-{{ $user->id }}">
                                    {{-- View Details --}}
                                    <x-ui.context-menu.item href="{{ route('manage-accounts.show', $user->id) }}">
                                    ...
                                    @if(in_array($currentUser->role->name, ['owner', 'admin']))
                                        <x-ui.context-menu.item href="{{ route('manage-accounts.edit', $user->id) }}">
                                            <svg class="h-5 mr-3" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    @if($currentUser->role->name === 'owner' && $user->id !== $currentUser->id)
                                        <x-ui.context-menu.divider />
                                        <x-ui.context-menu.item 
                                            href="{{ route('manage-accounts.destroy', $user->id) }}"
                                            method="DELETE"
                                            confirm="Hapus {{ $user->name }}? Aksi ini tidak dapat dibatalkan!"
                                            class="text-red-400">
                                            <svg class="h-5 mr-3" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
